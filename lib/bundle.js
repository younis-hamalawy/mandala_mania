...
var DrawPad = function () {
  ...
  _createClass(DrawPad, [{
    key: 'd2r',
    value: function d2r(deg) {
      return deg * Math.PI / 180;
    }
  }, {
    key: 'rotate',
    value: function rotate(p1, p2, a) {
      a = this.d2r(a);
    ...
    key: 'lineStroke',
    value: function lineStroke(start, end) {
      this.drawArea.graphics
      // .clear()
      .setStrokeStyle(this.stroke, 'round', 'round').beginStroke(this.color).moveTo(start.x, start.y).curveTo(this.oldPt.x, this.oldPt.y, this.oldMidPt.x, this.oldMidPt.y);
    }
  }, {
    key: 'init',
    value: function init() {
    ...
    value: function MouseMove(event) {
      ...
      for (var i = 0; i < this.slices - 1; i++) {
        ...
        var rP = this.rotate({ x: this.oldPt.x, y: this.oldPt.y }, this.center, this.start);
        var rC = this.rotate({ x: this.oldMidPt.x, y: this.oldMidPt.y }, this.center, this.start);
